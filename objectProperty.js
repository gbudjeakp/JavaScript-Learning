...
const saveDataLS = () => {
  ...
  window.localStorage.setItem('pirpleAlarms', JSON.stringify(alarmsToSave))
}

const readDataLS = () => {
  const savedAlarms = JSON.parse(window.localStorage.getItem('pirpleAlarms'))

  // parse timestamps to be date objects
  for (let i = 0; i < savedAlarms.length; i++) {
    const alarmTemp = savedAlarms[i]
...
const saveDate = (e) => {
  ...
  const date = new Date(year.value, month.value, day.value, hour.value, minute.value)
  const now = new Date()
  const triggeredStatus = (date.getTime() >= now.getTime()) ? 0 : 1
  ...
  const alarm = { date: date, triggered: triggeredStatus }
...
const notificationMessage = (time) => {
  if (!('Notification' in window)) {
    window.alert("This browser doesn't support the Notification API")
